...
$app->delete('/deletarReserva/{id}','DeleteReservaId');
...
function DeleteReservaId(Request $request, Response $response, array $args)
{
        $id = $args['id'];      
        $conn = getConn();
        $sql = "DELETE FROM tb_servico WHERE cd_servico=:id";
...
function postComentario(Request $request, Response $response, array $args)
{

    $dataCadastro = $request->getParsedBody();
    $db = getConn();
    $stmt = $db->prepare('INSERT INTO tb_comentarios (nome, comentario) VALUES (:nome, :comentario)');
    $stmt->execute([
        ':nome' => $dataCadastro['nome'],
        ':comentario' => $dataCadastro['comentario'],
